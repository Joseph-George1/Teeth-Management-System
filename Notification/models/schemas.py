...
class DeviceTokenRequest(BaseModel):
    ...
    IMPORTANT: user_id must come from backend login response, NOT generated by client
    
    Step 1: Mobile app logs in → gets user_id from Java backend
    Step 2: Mobile app registers device token with that user_id
    Step 3: Backend sends notifications to same user_id
    Step 4: Notifications delivered to device
    """
    user_id: int = Field(..., description="User ID from backend login (REQUIRED - NOT optional)")
